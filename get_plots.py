...
############################################################################
#================== Parameters and Their function values ==================#
############################################################################
nz=3; nl=200; zlst_ratio=linspace(2,3,nz); l_lst=logspace(-4,2,nl);
fig11=plt.figure(figsize=(8,2),dpi=200);	plt.subplots_adjust(wspace=0.40,hspace=0.4) # Re
fig12=plt.figure(figsize=(5,2),dpi=200); 	plt.subplots_adjust(wspace=0.40,hspace=0.4) # We
fig13=plt.figure(figsize=(5.5,4.5),dpi=200);plt.subplots_adjust(wspace=0.35,hspace=0.35) # Fr

color_lst=['black','red','blue']
style_lst=['-','--','dotted']
fig11.suptitle(r"$k_t={:.2f}m^2/s^2, \ \varepsilon={:.2f}m^2/s^3$".format(kt,et),fontsize=fs,y=1.0)
