...
def Turb_entrainment(Nx,Nt,kt,et,g,rhoc,rhod,sigma,z,nuc,nut,G,D,Dg,dt,Sl0,Table,model,zlam_min,zlam_max,wmeth,Fr2_crt_PolyExtra,F_tab_NP,sector):
	t1=t.time()
	for i in range(Nx):
		if z[i] > 0.5 :
			nut[i] = nuc
	...
	for i in range(Nx):
		...
		if model == 2:
			# New model
			J=Jent_numerical_New(kt[i],et[i],nuc,g,rhoc,sigma,Table,zlam_min,zlam_max,wmeth,Fr2_crt_PolyExtra,F_tab_NP,sector)
			S[i] = J/v0;
